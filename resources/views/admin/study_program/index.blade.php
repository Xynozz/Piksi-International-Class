        ...
        <h5 class="card-header">
            <button type="button" class="btn rounded-pill btn-info" data-bs-toggle="modal" data-bs-target="#createModal"
                ...
                <i class="bi bi-person-fill-add" data-bs-toggle="tooltip" data-bs-offset="0,4" data-bs-placement="left"
                    data-bs-html="true" title="Add Study Program"></i>
                Add Study Program
            </button>
            ...
            <form action="{{ route('StudyProgram.store') }}" method="POST">
                ...
                <div class="modal-body">
                    <div class="row">
                        ...
                        <div class="col-12 mb-3">
                            <label for="nameBasic" class="form-label">Head of Study</label>
                            <select class="form-select" name="lecture_id" required>
                                <option selected disabled value="" selected disabled>Select Lecture</option>
                                @foreach ($lecture as $data)
                                <option value="{{ $data->id }}">{{ $data->lecture_name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-12 mb-3">
                ...
                <div class="modal-body">
                    <div class="row">
                        ...
                        <div class="col-12 mb-3">
                            <label for="nameBasic" class="form-label">Head of Study</label>
                            <select class="form-select" name="lecture_id" required>
                                <option selected disabled value="" selected disabled>{{  $data->lecture_name }}</option>
                                @foreach ($lecture as $data)
                                <option value="{{ $data->id }}">{{ $data->lecture_name }}</option>
                                @endforeach
                            </select>
